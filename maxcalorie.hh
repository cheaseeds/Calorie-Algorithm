...
// TODO: implement this function, then delete the return statement below
  std::unique_ptr<FoodVector> vec(new FoodVector);
  
  for (unsigned int i = 0; i < source.size() && (*vec).size() < total_size; i++) {
    if (source[i]->foodCalories() >= min_calories && source[i]->foodCalories() <= max_calories) {
...
)
{
  ...
  while (!todo->empty()) {
    ...
    for (int i = 0; i < (int)todo->size(); i++) {
      if (weight < todo->at(i)->weight()) {
        weight = todo->at(i)->weight();
        index = i;
      }
    }
    q = todo->at(index)->foodCalories();   
    todo->erase(todo->begin() + index);  
    if ((result_weight + q) <= total_weight) {
      result_weight += q;
...
)
{
  ...
  double total_weight_cand, total_cal_cand;
  double total_weight_best, total_cal_best;
  ...
  for (uint64_t bits = 0; bits < pow(2, n); bits++) {
    ...
    sum_food_vector(*candidate, total_weight_cand, total_cal_cand);
    if (total_weight_cand <= total_weight) {
      sum_food_vector(*best, total_weight_best, total_cal_best );
      if (best->empty() || total_cal_cand > total_cal_best) {
        best = move(candidate);
      }
    }
  }
